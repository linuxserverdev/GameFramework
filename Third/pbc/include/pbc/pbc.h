...
inline const char* pbc_rmessage_safe_string(struct pbc_rmessage* r, const char* key, int index, int* sz)
{
	static const char* _s = "";
	const char* s = pbc_rmessage_string(r, key, index, sz);
	return s ? s : _s;
}
inline uint64_t pbc_rmessage_integer64(struct pbc_rmessage* pbr, const char* key, int index)
{
	uint32_t hight = 0;
	uint32_t low = pbc_rmessage_integer(pbr, key, index, &hight);
	return (uint64_t(hight) << 32) | low;
}
...
inline int pbc_wmessage_integer64(struct pbc_wmessage* pbw, const char* key, uint64_t value)
{
	return pbc_wmessage_integer(pbw, key, uint32_t(value & 0xFFFFFFFF), uint32_t(value >> 32));
}
// array api 
...
#define safe_delete_pbc_rmessage(m) if (m) pbc_rmessage_delete(m);
#define safe_delete_pbc_wmessage(m) if (m) pbc_wmessage_delete(m);
