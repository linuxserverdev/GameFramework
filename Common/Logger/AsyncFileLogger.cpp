...
AsyncFileLogger &AsyncFileLogger::getIns()
{
...
{
#ifdef __linux__
    prctl(PR_SET_NAME, fileBaseName_.c_str());
#endif
    while (!stopFlag_)
    {
        {
      ...
      fileExtName_(fileExtName)
{
    ...
    writeLog(StringPtr(new std::string("format pattern : "
             "{local time} - {thread id(SYS_gettid)} - {log level} - {log msg} - {source file name:source file line}\n")));
}
...
AsyncFileLogger::LoggerFile::~LoggerFile()
{
    if (fp_)
    {
        ...
        std::string newName =
            ...
            creationDate_.toCustomedFormattedStringLocal("%Y%m%d-%H%M%S") +
