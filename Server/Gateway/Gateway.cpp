...
void SocketReactorTest::testDataCollection()
{
	...
	assert(DataServiceHandler::_data.size() == 4);
	data0.erase(data0.size() - 1);
	assert(DataServiceHandler::_data[0] == data0);
	data1.erase(data1.size() - 1);
	assert(DataServiceHandler::_data[1] == data1);
	data2.erase(data2.size() - 1);
	assert(DataServiceHandler::_data[2] == data2);
	assert(DataServiceHandler::_data[3].empty());
}
...
void SocketReactorTest::main()
{
	SocketReactorTest test;


	//test.testSetSocketReactor();
	//test.testParallelSocketReactor();
	//test.testSocketConnectorFail();
	//test.testSocketConnectorTimeout();
...
int main()
{
	SocketAddress ssa("127.0.0.1", 8080);
	SocketAddress ssb("127.0.0.1", 8081);
	ServerSocket ss1(ssa);
	ServerSocket ss2(ssb);
	SocketReactor reactor;
	SocketAcceptor<EchoServiceHandler> acceptor1(ss1, reactor);
	SocketAcceptor<EchoServiceHandler> acceptor2(ss2, reactor);
	reactor.run();
	return 0;
}